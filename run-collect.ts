...
// Use a single tab for everything - siteinspire tab
const WS_URL = "ws://localhost:9222/devtools/page/72A4676F98D0BB50459A8EF1B0A97177";

function navigateAndScreenshot(url: string, filepath: string, wait = 5000): Promise<void> {
...
function scrollAndScreenshot(scrollY: number, filepath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(WS_URL);
    ws.on("open", () => {
...
async function collectForStyle(style: StyleConfig) {
  ...
  try {
    await navigateAndScreenshot(`https://same.energy/search?q=${encodeURIComponent(style.sameQuery)}`, path.join(base, "same-1.png"), 6000);
    await scrollAndScreenshot(800, path.join(base, "same-2.png"));
    await scrollAndScreenshot(1600, path.join(base, "same-3.png"));
  } catch (e) { console.log(`  same energy error: ${e}`); }

  // 2. Are.na search + scroll
  try {
    await navigateAndScreenshot(`https://www.are.na/search/${encodeURIComponent(style.arenaQuery)}`, path.join(base, "arena-1.png"), 6000);
    await scrollAndScreenshot(800, path.join(base, "arena-2.png"));
  } catch (e) { console.log(`  arena error: ${e}`); }

  // 3. Cosmos explore
  try {
    await navigateAndScreenshot("https://www.cosmos.so/explore", path.join(base, "cosmos-1.png"), 5000);
  } catch (e) { console.log(`  cosmos error: ${e}`); }

  // 4. Real reference sites
  for (let i = 0; i < style.realSites.length; i++) {
    try {
      await navigateAndScreenshot(style.realSites[i], path.join(base, `ref-${i + 1}.png`), 6000);
    } catch (e) { console.log(`  ref site error (${style.realSites[i]}): ${e}`); }
  }
}
